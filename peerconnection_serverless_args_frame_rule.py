...
parser = argparse.ArgumentParser(description='frame_rule_based test')

parser.add_argument('--portNum', dest='portNum', type=int, default=8000)
parser.add_argument('--traceNum', dest='traceNum', type=int, default=34)
...
def appRecvProxy(argv, allFrame, recv_send_pipe, cc, RL_Pipe):
    ...
    flag, recvRateTrack, delayTrack, lossTrack, PSNRTrack, FrameDelayTrack, FrameSkipTrack, timeTrack = processR(
        appRecv.stdout, appRecv.stdin, recvf, decFrame, completeFrame, renderFrame, allFrame, estimator, packet_record,
        cc, RL_Pipe)
    ...
    if flag:
        ...
        plotRecvT, plotRecv, mahiT, mahiRate = bitRateAnalysis2.getPLTlist(test_args.traceType, test_args.traceNum)
        plotResult.plt_track(plotTargetT, plotTarget, plotEncT, plotEnc, \
                             plotSetT, plotSet, plotRecvT, plotRecv, mahiT, mahiRate,
                             savePath + f'bitrateAnalysis_{test_args.traceType}_{test_args.traceNum}.jpg')
...
def processR(Rout, Rin, recvf, decFrame, completeFrame, renderFrame, allFrame, estimator, packet_record, cc, RL_Pipe):
    # global estimator
    ...
    while True:
        ...
        if request:
            ...
            RL_Pipe.send(frame_estimator.active_loss)
            print("frame_estimator.active_loss:", frame_estimator.active_loss)
...
def run(argv, cc):
    ...
    action_proxy_process = mp.Process(target=Action_Proxy, args=(SENDER_PIPE_PATH, Action_Proxy_Pipe))
    action_proxy_process.start()
    print("action_proxy_process start!")

    p1 = mp.Process(target=appRecvProxy, args=(argv[1], allFrame, recv_send_pipe, cc, RL_Pipe))
    ...
    # for i in allFrame:
    #     print(i)
    action_proxy_process.terminate()
    action_proxy_process.kill()
    print("finish")
