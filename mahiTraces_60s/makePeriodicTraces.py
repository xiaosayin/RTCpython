...
PERIO_TIME = 10  * 1000   # ms
TOTAL_TIME = 70 * 1000   # ms
...
def makeOneTrace(minT, maxT, minRate, maxRate, path, logPath, pngPath):
    ...
    logf.close()
    logf = open(logPath, "a+")
    logf.write(f"#t:{minT}-{maxT} ; rate:{minRate}-{maxRate}\n")
    ...
    while(now_time < PERIO_TIME):
        ...
        for i in intervalList:
            traceList.append(i + now_time)
            tracef.write(str(i + now_time)+'\n')
        logList.append("rate: %d \t duration: %d \n" %(random_rate, intervalList[-1]))
        logf.write("rate: %d \t duration: %d \n" %(random_rate, intervalList[-1]))
    ...
    if TOTAL_TIME % PERIO_TIME == 0:
        restPeriodicNum = int(TOTAL_TIME / PERIO_TIME) - 1
    else:
        restPeriodicNum = int(TOTAL_TIME / PERIO_TIME)
    for i in range(0, restPeriodicNum):
        time_base += traceList[-1]
        for t in traceList:
            tracef.write(str(t + time_base)+'\n')
        for l in logList:
            logf.write(l)
    ...
    for i in range(0, restPeriodicNum):
        j = 0
        while j < len(tmpTime_t):
            tmpTime_t[j] += traceList[-1] / 1000
            j += 1
        time_t += tmpTime_t
        rate_t += tmpRate_t
...
def main():
    d, rates, times = makeIntervalTrace(6000, 40)
    for i in d:
    ...
    plt.plot([times[0], times[-1]], [6000, 6000])
    plt.grid()
    #plt.show()
    
    os.system("rm mahiTraces/periodic/*")
    for traceN in range(0,traceNum):
        ...
        r.sort()
        tracePath = f"mahiTraces/periodic/trace{traceN}.trace"
        logPath = f"mahiTraces/periodic/trace{traceN}.log"
        pngPath = f"mahiTraces/periodic/trace{traceN}.png"
