...
class GymEnv:
    ...
    def reset(self, type, traceN, queLength, lossRate):
        self.processStat = ProcessStat(historyLength)
        trace_dir = os.path.join(os.path.dirname(__file__), "traces")
        self.trace_set = glob.glob(f'{trace_dir}/**/*.json', recursive=True)
        self.action_space = spaces.Box(low=0.0, high=1.0, shape=(1,), dtype=np.float64)
        self.observation_space = spaces.Box(
        ...
        print("trace: ", f"mm-link mahiTraces/{type}/trace{traceN}.trace 12mbps.trace --uplink-queue=droptail --uplink-queue-args=\"packets={queLength}\" mm-loss uplink {lossRate/100} ")
        args = [f"mm-link mahiTraces/{type}/trace{traceN}.trace 12mbps.trace --uplink-queue=droptail --uplink-queue-args=\"packets={queLength}\" mm-loss uplink {lossRate/100} " + \
            f"-- sh ./rtcGym/alphartc_gym/sh/pcsend{self.portNum}.sh", \
                f"./rtcGym/alphartc_gym/json/receiver_pyinfer{self.portNum}.json"]
    ...
    def step(self, action):
        ...
        if flag:
            ...
            if len(statePsnr) == historyLength:
                ...
                rewardPSNR = (float(statePsnr[-1]/600000)) * 4
        ...
        #reward = receiving_rate / 1000000.0 - delay * 16 / 1000.0 - 6 * burstLoss_ratio + rewardFrameDelay #+ encLoss
        reward = rewardPSNR - delay * 16 / 1000.0 - 6 * burstLoss_ratio + rewardFrameDelay #+ encLoss
        ...
        return states, reward, rewardPSNR, - delay * 16 / 1000.0  , - 6 * burstLoss_ratio, rewardFrameDelay, done, \
