...
all_avg_file = "/run/media/yinwenpei/data_of_RTC/frame_rule_test_files/all_avg.txt"
...
def one_test(portNum, traceNum, traceType, video_id, queLength, lossRate):
    ...
    RTC_process = subprocess.Popen(
        f"python3 peerconnection_serverless_args_frame_rule.py --portNum={portNum} --traceNum={traceNum} --traceType={traceType} \
...
def calculate_avg():
    ...
    for i in range(len(record_traceNum)):
        readPath = f"/run/media/yinwenpei/data_of_RTC/frame_rule_test_files/{record_traceType[i]}_{record_traceNum[i]}_video_id{record_video_id[i]}_queLength{record_queLength[i]}_lossRate{record_lossRate[i]}/"
...
def main():
    ...
    # one_test(portNum=8000, traceNum=496, traceType='random_2M', video_id=1, queLength=219, lossRate=2.22)
