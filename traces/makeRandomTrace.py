...
def makeOneTrace(minT, maxT, minRate, maxRate, minLoss, maxLoss, minDelay, maxDelay, totalTime, traceN):
    #tracePath = f"random/trace_t_{t[0]}-{t[1]}_rate_{r[0]}-{r[1]}_loss_{l[0]}-{l[1]}_delay_{d[0]}-{d[1]}.py"
    tracePath = f"random/trace{traceN}.py"
    ...
    tracef.write("def link():\n")
    tracef.write(f"\ttracef = open(\"traces/random/trace{traceN}.txt\", 'w')\n")
    tracef.write(f"\ttracef.close()\n")
    tracef.write(f"\ttracef = open(\"traces/random/trace{traceN}.txt\", 'a+')\n")
    tabs += 1
    curT = 0    #currentTime
