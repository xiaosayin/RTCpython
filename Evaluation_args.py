...
Average_recode_file = savePath + "RL_Average_record.txt"
...
def VQA():
    ...
    score1 = subprocess.Popen(
        f"/usr/bin/ffmpeg -r 30 -s 1280x720 -pix_fmt yuv420p -i result/cut.yuv -r 30 -s 1280x720 -pix_fmt yuv420p -i {output_yuv} -vframes {int(frameNum)} -filter_complex {method} -f null -", \
    ...
    score2 = subprocess.Popen(
        f"/usr/bin/ffmpeg -r 30 -s 1280x720 -pix_fmt yuv420p -i result/cut.yuv -r 30 -s 1280x720 -pix_fmt yuv420p -i {output_yuv} -vframes {int(frameNum)} -filter_complex {'psnr'} -f null -", \
        shell=True, stderr=subprocess.PIPE, encoding='utf-8')
...
def getFrameDelay(allFrame, renderFrame):
    ...
    plt.savefig(savePath + "frame_delay.png")
    return frame_delay
...
def getPSNR(allFrame, renderFrame):
    ...
    plt.grid()
    plt.savefig(savePath + "Enc_PSNR.png")
    return PSNR
