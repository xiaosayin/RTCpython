...
def getPLTlist(traceName):
    ...
    mahiRate, mahiT = drawMahiRate.drawMahiTrace(f"sampledTraces/chosenTraces/{traceName}.log")
    # ==================================
    ## get the whole pack
    plotTarget = []
    plotEnc = []
...
def main():
    ...
    plt.step(mahiT, mahiRate, alpha = 0.5, label='recv rate', color='purple')
    plt.legend(loc=1)
    plt.xlabel("t (us)")
