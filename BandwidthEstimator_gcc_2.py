...
class GCCEstimator(object):
    def __init__(self):
        ...
        self.state = 'Hold'
        self.last_bandwidth_estimation = 1000 * 1000
    ...
    def report_states(self, stats: dict):
        ...
        self.allSize += packet_info.size
        printDebug(f"allSize: {self.allSize}")
    ...
    def get_estimated_bandwidth(self) -> int:
        ...
        '''
        print("before getLine: ", round(time.time() * 1000))
        sendLine = f.readline()
        print(sendLine)
        print("after getLine: ", round(time.time() * 1000))
        ...
        printDebug(f"{int(BWE_by_delay)} ; {int(BWE_by_loss)}")
        if flag == True:
            self.packets_list = []
        self.last_bandwidth_estimation = bandwidth_estimation
        printDebug(f"estimate time: {round(time.time() * 1000) - t1}")
        #return bandwidth_estimation,self.overuse_flag
        #bandwidth_estimation = 585 * 1000
        #bandwidth_estimation = random.randint(50*1000, 2000*1000)
