...
parser = argparse.ArgumentParser(description='base test')

parser.add_argument('--portNum', dest='portNum', type=int, default=8000)
parser.add_argument('--traceNum', dest='traceNum', type=int, default=38)
parser.add_argument('--traceType', dest='traceType', type=str, default='random')
parser.add_argument('--video_id', dest='video_id', type=int, default=1)
parser.add_argument('--queLength', dest='queLength', type=int, default=168)
parser.add_argument('--lossRate', dest='lossRate', type=float, default=3)
# parser.add_argument('--savePath', dest='savePath', type=str, default=None)

test_args = parser.parse_args()
savePath = f"/run/media/yinwenpei/data_of_RTC/base_test_files/{test_args.traceType}_{test_args.traceNum}_video_id{test_args.video_id}_queLength{test_args.queLength}_lossRate{test_args.lossRate}/"

portNum = test_args.portNum
traceNum = test_args.traceNum
queLength = test_args.queLength
lossRate = test_args.lossRate
video_id = test_args.video_id
videos = ["Johnny", "KristenAndSara", "vidyo1", "vidyo3", "FourPeople"]
traceType = test_args.traceType

houzhui = f'{traceType}_{traceNum}_{queLength}_{lossRate}_{videos[video_id]}'

Average_recode_file = savePath + "Base_Average_record.txt"
...
def VQA():
    ...
    # method = "psnr"
    frameNum = os.popen(
        f"/usr/bin/ffprobe -v error -count_frames -select_streams v:0 -show_entries stream=nb_read_frames -of default=nokey=1:noprint_wrappers=1 {output_path}").read()
...
def RGBVQA():
    ...
    frameNum = os.popen(
        f"/usr/bin/ffprobe -v error -count_frames -select_streams v:0 -show_entries stream=nb_read_frames -of default=nokey=1:noprint_wrappers=1 ./result/base_outvideo.avi").read()
    RGB_Vmaf = os.popen(
        f"/usr/bin/ffmpeg -r 30 -i result/base_cut.avi -r 30 -i ./result/base_outvideo.avi -vframes {int(frameNum)} -filter_complex {method} -f null -").read()
    RGB_PSNR = os.popen(
        f"/usr/bin/ffmpeg -r 30 -i result/base_cut.avi -r 30 -i ./result/base_outvideo.avi -vframes {int(frameNum)} -filter_complex {'psnr'} -f null -").read()
...
def getAllFrame(allFrame):
    ...
    while sendLine:
        ...
        if "YINWENPEI: Before enc" in sendLine:
            ...
            frameCnt = int(sendLine[sendLine.index("ID") + 3:sendLine.index(",")])
            allFrame[frameCnt * timelistL + NumI] = frameCnt
            allFrame[frameCnt * timelistL + widthI] = int(
                sendLine[sendLine.index("width") + 6:sendLine.index("height") - 2])
            allFrame[frameCnt * timelistL + heightI] = int(
                sendLine[sendLine.index("height") + 7:sendLine.index("at") - 2])
            allFrame[frameCnt * timelistL + beforeEncTI] = int((getTime(sendLine, timeLog) / 1000)) % 1000000

        if "YINWENPEI: PSNR" in sendLine:
            allFrame[frameCnt * timelistL + psnrI] = int(
                float(sendLine[sendLine.index("AVERAGE: ") + len("AVERAGE: "):]) * 10000)
    ...
    recvf.close()
    return allFrame, completeFrame, renderFrame, last_frameCnt
...
def getFrameDelay(allFrame, completeFrame):
    ...
    plt.savefig(savePath + "frame_delay" + houzhui + ".png")
    return frame_delay
...
def getPSNR(allFrame, renderFrame):
    ...
    plt.grid()
    plt.savefig(savePath + "Enc_PSNR_" + houzhui + ".png")
    return PSNR
...
allFrame, completeFrame, renderFrame, last_frameCnt = getAllFrame(allFrame)
...
pd_frame_delay = pd.DataFrame({"frame_delay": frame_delay})
pd_frame_delay.to_csv(savePath + "frame_delay.csv", mode="w+", index=False, sep='\t', encoding='utf-8')
print("average_frame_delay: ", sum(frame_delay) / len(frame_delay))

print("frame_loss_rate:", len(drop_frame_id) / len(whole_list))
print("new_frame_loss_rate:", 1 - len(recv_video_frame_id)/last_frameCnt)
...
with open(Average_recode_file, "w") as f:
    ...
    f.write("new frame_loss_rate: " + str(1 - len(recv_video_frame_id)/last_frameCnt) + "\n")
