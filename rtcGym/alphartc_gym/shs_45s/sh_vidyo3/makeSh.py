...
while portNum < 8000 + 121:
    ...
    while recv8000Line:
        if "./peerconnection_serverless.origin " not in recv8000Line:
            ...
            # newrecvf.write(f"./peerconnection_serverless.origin ./rtcGym/alphartc_gym/jsons_45s/json_vidyo3/sender_pyinfer{portNum}.json\n")
            newrecvf.write(
                  f"./peerconnection_serverless.origin ./rtcGym/alphartc_gym/jsons_45s/json_vidyo3/receiver_pyinfer{portNum}.json\n")
        recv8000Line = recvf8000.readline()
    newrecvf.close()
    recvf8000.close()
    portNum += 1