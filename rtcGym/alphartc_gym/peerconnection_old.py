...
def processR(Rout, Rin, recvf, decFrame, completeFrame, inI, decI, renI, addI, writI, allFrame, writDrop, pipe, appRecv):
    ...
    while True:
        ...
        if request:
            ...
            bwe = pipe.recv()
            #bwe = bandwidth
            #bwe = random.randint(3000000, 3100000)
            '''
            bwe = 9750 * tmpCnt + 100000
            tmpCnt += 1
            '''
            '''
            if startTime == None:
            ...
            print("curBWETime: ", (round(time.time()) - startTime) / 10 % 2)
            if 0 <= (round(time.time()) - startTime) / 10 % 2 and \
                 (round(time.time()) - startTime) / 10 % 2 <= 1.1:
                bwe = 650000
            elif 1.9 <= (round(time.time()) - startTime) / 10 % 2 and \
                 (round(time.time()) - startTime) / 10 % 2 <= 1.999999:
                bwe = 650000
            else:
                bwe = 1550000
            '''
            if bwe < 50000 or bwe > 4000000:    #bwe out of range!kill!
                return False
