...
TEST_traceNum = test_args.traceNum
TEST_Que = test_args.queLength
TEST_Loss = test_args.lossRate  # %
TEST_VIDEO = test_args.video_id  # [Johnny, KristenAndSara, vidyo1, vidyo3, FourPeople]
...
def main():
    ...
    ppo = PPO(state_dim, state_length, action_dim, exploration_param, lrMax, lrMax, lrMin, max_num_episodes, betas,
              gamma, K_epochs, ppo_clip)
    ...
    for episode in range(max_num_episodes):
        ...
        while True:
            ...
            if done and sample_cnt > sample_Maxnum:
                ...
                drawPlt('stateNum', 'statePacketDelay', f'{data_path}state1PacketDelay_{currTime}.jpg',
                        statePacketDelay)
                drawPlt('stateNum', 'statePacketDelayGradient', f'{data_path}state2PacketDelayGradient_{currTime}.jpg',
                        statePacketDelayGradient)
                drawPlt('stateNum', 'statePacketLoss', f'{data_path}state3PacketLoss_{currTime}.jpg', statePacketLoss)
                drawPlt('stateNum', 'statePacketBurstLoss', f'{data_path}state4PacketBurstLoss_{currTime}.jpg',
                        statePacketBurstLoss)
                ...
                drawPlt('stateNum', 'stateFrameDelayGradient', f'{data_path}state7FrameDelayGradient_{currTime}.jpg',
                        stateFrameDelayGradient)
